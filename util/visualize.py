...
class Visualizer():
    ...
    def print_current_losses(self, epoch, iters, losses, t_comp, t_data):
        ...
        Parameters:
            ...
            losses (OrderedDict) -- training losses stored in the format of (name, float) pairs
            t_comp (float) -- computational time per dataset point (normalized by batch_size)
            t_data (float) -- dataset loading time per dataset point (normalized by batch_size)
        """
        message = '(epoch: %d, iters: %d, time: %.3f, dataset: %.3f) ' % (epoch, iters, t_comp, t_data)
